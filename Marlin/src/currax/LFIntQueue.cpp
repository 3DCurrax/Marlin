...
namespace LFIntQueue
{
   ...
   atomic<unsigned long > mWriteRetry;
   atomic<unsigned long > mReadRetry;
   atomic<unsigned long > mPopRetry;
   atomic<unsigned long > mPushRetry;

   unsigned long writeRetry() {return mWriteRetry.load();}
   unsigned long readRetry()  {return mReadRetry.load();}
   unsigned long popRetry()   {return mPopRetry.load();}
   unsigned long pushRetry()  {return mPushRetry.load();}
