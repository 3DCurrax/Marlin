...
void Header::headerReCopyToFrom  (Ris::ByteBuffer* aBuffer,BaseMsg* aParent)
{
   ...
   if (aBuffer->isCopyTo())
   {
      ...
      // Restore buffer parameters to the initial position.
      aBuffer->setPosition (mInitialPosition);
      aBuffer->setLength   (mInitialPosition);

      // Copy the adjusted header into the buffer at the original position.
      copyToFrom( aBuffer );

      // Restore buffer parameters to the final position.
